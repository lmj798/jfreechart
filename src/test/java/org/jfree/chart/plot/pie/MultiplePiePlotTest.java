...
public class MultiplePiePlotTest implements PlotChangeListener {
    ...
    @Test
    public void testSetPieChartExceptionMessageFormatForNullInput() {
        MultiplePiePlot plot = new MultiplePiePlot();
        ...
        assertTrue(message.startsWith("Null 'pieChart'"), 
                "Exception message should start with \"Null 'pieChart'\"");
        assertTrue(message.endsWith("' argument."), 
                "Exception message should end with \"' argument.\"");
    }
    
    @Test
    public void testSetPieChartExceptionMessageFormatForInvalidPlotType() {
        MultiplePiePlot plot = new MultiplePiePlot();
        
        DefaultCategoryDataset<String,String> dataset = new DefaultCategoryDataset<>();
        dataset.addValue(1.0, "R1", "C1");
        JFreeChart invalidChart = ChartFactory.createAreaChart("Test", "X", "Y", dataset);
        
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, 
                () -> plot.setPieChart(invalidChart), 
                "Should throw IllegalArgumentException for invalid plot type");
        
        String message = exception.getMessage();
        assertNotNull(message, "Exception message should not be null");
        assertTrue(message.contains("PiePlot"), "Exception message should contain 'PiePlot'");
        assertTrue(message.contains("pieChart"), "Exception message should contain 'pieChart'");
    }
